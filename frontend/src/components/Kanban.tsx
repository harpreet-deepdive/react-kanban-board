...
import kanbanBoards from "../mock/kanban.json";
import { ReactSortable } from "react-sortablejs";

import { Button, Modal, Input, IconButton } from "rsuite";
import {
  HiArrowsExpand,
  HiClipboard,
  HiClipboardCopy,
...
const KanbanPage: FC = function () {
  ...
  const barColors = ["#1FD191", "#FED01C", "#607EF0", "#D050FE"];

  return (
    <div className={`${sidebarCtx.expanded === true ? "ml-60" : "ml-10"}`}>
      <div className="inline-block   align-middle">
        <ActionBar />
        <div className="w-[100%] mt-10  mb-6 flex items-start justify-start space-x-4 px-4">
          {list.map((board, i) => (
            <div key={board.id}>
...
const ActionBar: FC = () => {
  ...
  return (
    <div
      className={`fixed backdrop-blur-8  p-2  ${
        sidebarCtx.expanded ? "w-[73rem]" : "w-[95%]"
      }  pl-4`}
    >
      <div className="flex justify-between ">
        <div>
          <h3 className="text-xl text-black font-extrabold">Board Name</h3>
        </div>
        <div className="flex">
          <Button
            color="blue"
          ...
          <Button
            color="blue"
            className="text-black border-black ml-2"
            appearance="ghost"
            startIcon={<HiArrowsExpand />}
          >
            Create Board
    ...
    <>
      ...
      <Modal onClose={() => setOpen(false)} open={isOpen}>
        <Modal.Header className="border-b border-gray-200 !p-6 dark:border-gray-700">
          <strong className="text-xl font-extrabold">Edit task</strong>
        </Modal.Header>
        <Modal.Body>
          ...
          <div className="mb-5 flex flex-col items-start justify-center space-y-3">
            ...
            <div className="flex flex-row flex-wrap">
              ...
              <Button className="ml-5 font-bold dark:bg-gray-600 [&>*]:py-1">
                <div className="flex items-center gap-x-2 text-xs">
                  <HiPlus />
                  Join
                </div>
              </Button>
              <Button className="ml-3 font-bold dark:bg-gray-600 [&>*]:py-1">
                <div className="flex items-center gap-x-2 text-xs">
                  <HiPaperClip />
                  Attachment
                </div>
